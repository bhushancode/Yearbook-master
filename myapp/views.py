...
def profile(request):
    ...
    print(email)
    print("InProfile")
    exceptions = ['vidya.bhushan.che19@itbhu.ac.in']
    ...
    # u.save()
    #dy to render some option in profile based on graduating year
    dy=True
    if u.graduating_year!=2021:
        dy=False
    if request.method == 'GET':
        context = {"user": u,"dy":dy}
        return render(request, 'myapp/profile.html', context)
    # print int(request.FILES.get('dp').size)<6000000
    if(request.FILES.get('dp') != None):
        if( int(request.FILES.get('dp').size) > 2000000 ):
            return render(request, 'myapp/profile.html', {"user": u, "image": "Image size should be less than 2mb", "dy":dy})
        # Get the picture
        picture = request.FILES.get('dp')
        # check extension
        if not (picture.name.lower().endswith(('.png', '.jpg', '.jpeg'))):
            return render(request, 'myapp/profile.html', {"user": u,"dy":dy, "image": "Image should be in .png, .jpg or .jpeg format"})
    ...
    if(request.FILES.get('genPic1') != None):
        if( int(request.FILES.get('genPic1').size) > 2000000 ):
            return render(request, 'myapp/profile.html', {"user": u,"dy":dy ,"image": "Image size should be less than 2mb"})
    ...
    if len(u.name) == 0:
        return render(request, 'myapp/profile.html', {"user": u,"dy":dy, "name": "Name cannot be empty"})
